...
public class ReviewAuthorizationFilter extends OncePerRequestFilter {
  ...
  private static final Pattern REVIEW_TASK_DETAIL = Pattern.compile("^/api/review-tasks/[^/]+$");
  ...
  protected boolean shouldNotFilter(HttpServletRequest request) {
    ...
    boolean reviewApi =
      "/api/reports/review".equals(path) ||
  ...
  protected void doFilterInternal(
    ...
    response.setCharacterEncoding(StandardCharsets.UTF_8.name());
    response.setContentType(MediaType.APPLICATION_JSON_VALUE);
    objectMapper.writeValue(response.getWriter(), new ApiResponse<>(403, "无权查看审核报告", null));
  }
}
