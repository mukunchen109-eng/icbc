...
public class DifyService {
  ...
  public String generateDailySummary(DifyWorkflowRequest workflowRequest) {
    ...
    try {
      ...
      throw new ResponseStatusException(
        HttpStatus.BAD_GATEWAY,
        "Dify workflow call failed: " + ex.getMessage(),
        ex
      );
    }
  }
  ...
  private RestClient buildRestClient(ReportProperties.Dify dify, MediaType acceptType) {
    ...
    requestFactory.setConnectTimeout(
      Duration.ofSeconds(Math.max(1, dify.getConnectTimeoutSeconds()))
    );
  ...
  private String readStreamingResponse(ReportProperties.Dify dify, ClientHttpResponse response)
    throws IOException {
    if (response.getStatusCode().isError()) {
      ...
      throw new ResponseStatusException(
        HttpStatus.BAD_GATEWAY,
        "Dify Workflow HTTP error: status=" +
          response.getStatusCode().value() +
          ", " +
          response.getStatusText()
      );
  ...
  private String extractStreamingContent(InputStream body) throws IOException {
    ...
    if (StringUtils.hasText(result.error)) {
      throw new ResponseStatusException(
        ...
        "Dify workflow execution failed" +
          buildTraceSuffix(result.taskId, result.workflowRunId) +
          ", error=" +
          result.error
      );
    }
    ...
    throw new ResponseStatusException(
      HttpStatus.BAD_GATEWAY,
      "Cannot parse Dify streaming workflow response" +
        buildTraceSuffix(result.taskId, result.workflowRunId)
    );
  }

  private void handleStreamingEvent(String event, String data, StreamingWorkflowResult result)
    throws IOException {
  ...
  private String extractContent(Map<String, Object> response) {
    if (response == null || response.isEmpty()) {
      throw new ResponseStatusException(
        HttpStatus.BAD_GATEWAY,
        "Dify workflow returned an empty response"
      );
    }
    ...
    if (data instanceof Map<?, ?> dataMap) {
      ...
      if (status instanceof String workflowStatus && "failed".equalsIgnoreCase(workflowStatus)) {
        ...
        throw new ResponseStatusException(
          HttpStatus.BAD_GATEWAY,
          "Dify workflow execution failed" +
            buildTraceSuffix(taskId, workflowRunId) +
  ...
  private String previewArticles(List<DifyNewsArticleInput> articles) {
    ...
    for (int i = 0; i < previewSize; i++) {
      ...
      builder
        .append("{id=")
        .append(preview(article.id()))
        .append(", title=")
        .append(preview(article.title()))
        .append(", industry=")
        .append(preview(article.industry()))
        .append(", area=")
        .append(preview(article.area()))
        .append("}");
    }
