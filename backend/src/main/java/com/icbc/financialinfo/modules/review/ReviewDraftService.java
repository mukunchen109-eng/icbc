...
public class ReviewDraftService {
  ...
  public SaveDraftResult save(long reportId, long userId, String role, SaveDraftRequest request) {
    ...
    for (DraftOperation operation : operations) {
      ...
      switch (type) {
        case "MODIFY" -> reportStatus = commandService
          .modifyArticle(
            reportId,
            required(operation.articleId(), "报告条目不能为空"),
            userId,
            role,
            new ModifyArticleRequest(
              operation.title(),
              operation.summaryContent(),
              operation.reason()
            )
          )
          .reportStatus();
        case "MARK_MODIFY" -> reportStatus = commandService
            .addMark(
              reportId,
              userId,
              role,
          ...
          null
          ? reportStatus
          : reviewingStatus(role);
        case "COMMENT" -> reportStatus = commandService
            .addComment(
              reportId,
              userId,
              role,
          ...
          ? reportStatus
          : reviewingStatus(role);
        case "REPLACE" -> reportStatus = commandService
          .replaceArticle(
            reportId,
            required(operation.articleId(), "报告条目不能为空"),
            userId,
            role,
            new ReplaceArticleRequest(
              required(operation.newNewsId(), "替换资讯不能为空"),
              operation.reason()
            )
          )
          .reportStatus();
        case "RESOLVE_ISSUE" -> issueService.resolve(
          required(operation.issueId(), "审核问题不能为空"),
          userId
        );
        default -> throw new ReviewOperationException(
