...
public final class DifySummaryArticleParser {
  ...
  private static void appendArticles(
    List<DifySummaryArticle> result,
    JsonNode articles,
    String selectType
  ) {
    ...
    for (JsonNode item : articles) {
      ...
      result.add(
        new DifySummaryArticle(newsId, category, title, summary, sourceLabel, reason, selectType)
      );
    }
  }
  ...
  private static JsonNode locateSelectionContainer(JsonNode node) {
    ...
    if (
      (selected != null && selected.isArray()) || (alternatives != null && alternatives.isArray())
    ) {
      return node;
    }
