...
public class PdfService {
  ...
  public Path writeDailySummary(
    ...
    String resolvedFileName = fileBaseName == null || fileBaseName.isBlank()
      ? FILE_NAME
      : fileBaseName.trim();
  ...
  private List<LineSpec> buildLines(
    String reportTitle,
    LocalDate reportDate,
    String content,
    PDFont font
    ...
    result.add(
      new LineSpec(
        "生成日期：" + reportDate.format(DATE_FORMATTER),
        font,
        META_FONT_SIZE,
        LEADING,
        0,
    ...
    for (Block block : ReportTextFormatter.format(content)) {
      switch (block.type()) {
        case BLANK -> result.add(
          new LineSpec("", font, BODY_FONT_SIZE, LEADING * 0.6F, 0, Color.BLACK)
        );
        case HEADING -> addWrappedBlock(
          result,
          block.text(),
          font,
          HEADING_FONT_SIZE,
        ...
        case SUBHEADING -> addWrappedBlock(
          result,
          block.text(),
          font,
