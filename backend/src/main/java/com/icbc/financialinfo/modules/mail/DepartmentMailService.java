...
public class DepartmentMailService {
    private static final Set<String> SENDABLE_STATUSES = Set.of("PENDING", "FAILED", "PARTIAL_FAILED");
    ...
    public DepartmentMailService(JdbcTemplate jdbc,
    ...
    public List<RecipientOption> listRecipients() {
        return jdbc.query("SELECT id,recipient_name,recipient_email FROM mail_recipient ORDER BY id",
    ...
    public CreateResult create(long manager, CreateRequest request) {
        String reportStatus = jdbc.query("SELECT status FROM report WHERE id=?",
                        (rs, row) -> rs.getString(1), request.reportId()).stream().findFirst()
                .orElseThrow(() -> new DepartmentBusinessException(404, "报告不存在"));
        if (!"FINAL_APPROVED".equals(reportStatus))
        ...
        Integer sent = jdbc.queryForObject("""
                ...
                """, Integer.class, request.reportId());
        if (sent != null && sent > 0) throw new DepartmentBusinessException(409, "该报告正在发送，请勿重复操作");
        ...
        if (keys.getKey() == null) throw new DepartmentBusinessException(500, "数据库未返回邮件任务编号");
        ...
        for (Recipient recipient : request.recipients()) {
            jdbc.update("""
                    ...
                    """, taskId, recipient.name().trim(), recipient.email().trim().toLowerCase());
        }
    ...
    public SendResult send(long manager, long taskId) {
        ...
        if (!SENDABLE_STATUSES.contains(task.status()))
            throw new DepartmentBusinessException(409, "邮件任务已处理");

        PreparedArtifacts artifacts = archiveService.prepare(task.reportId());
        List<MailRecipientLog> pending = recipientLogs(taskId).stream()
                .filter(log -> !"SUCCESS".equals(log.status())).toList();
        if (pending.isEmpty() && recipientLogs(taskId).isEmpty())
    ...
    public MailLogDetail retry(long manager, long logId) {
        ...
        PreparedArtifacts artifacts = archiveService.prepare(task.reportId());
        jdbc.update("UPDATE mail_task SET mail_status='SENDING',completed_at=NULL WHERE id=?", task.id());
        deliver(task, log, artifacts, true);
        completeTask(task, manager, artifacts);
        return logs(manager, task.id()).stream().filter(item -> item.id() == logId).findFirst().orElseThrow();
    }

    private void deliver(MailTask task, MailRecipientLog log, PreparedArtifacts artifacts, boolean retry) {
        try {
            if (!mailEnabled) throw new IllegalStateException("真实邮件发送未启用，请设置 MAIL_ENABLED=true");
            if (mailSender == null || fromAddress.isBlank())
                throw new IllegalStateException("发件邮箱未配置，请设置 MAIL_HOST、MAIL_USERNAME、MAIL_PASSWORD 和 MAIL_FROM");
            ...
            jdbc.update("""
                    UPDATE mail_log SET mail_status='FAILED',retry_count=retry_count+?,error_message=?,
                     ...
                     WHERE id=?
                    """, retry ? 1 : 0, truncate(message(exception), 2000), log.id());
        }
    }
    ...
    private SendResult completeTask(MailTask task, long manager, PreparedArtifacts artifacts) {
        ...
        int success = (int) results.stream().filter(log -> "SUCCESS".equals(log.status())).count();
        int failed = total - success;
        String status = success == total && total > 0 ? "COMPLETED" : success == 0 ? "FAILED" : "PARTIAL_FAILED";
        try {
            archiveService.archive(task.reportId(), manager, artifacts);
        } catch (RuntimeException exception) {
            jdbc.update("UPDATE mail_task SET mail_status='PARTIAL_FAILED',completed_at=NULL WHERE id=?", task.id());
            throw exception;
        }
        if ("COMPLETED".equals(status)) {
            ...
            jdbc.update("UPDATE mail_task SET mail_status=?,completed_at=NULL WHERE id=?", status, task.id());
        }
        return new SendResult(task.id(), status, total, success, failed, artifacts.attachments(), logs(manager, task.id()));
    }
    ...
    public MailTaskDetail detail(long manager, long id) {
        ...
        return jdbc.query("""
                ...
                """, (rs, row) -> new MailTaskDetail(
                rs.getLong("id"), rs.getLong("report_id"), rs.getString("subject"),
                rs.getString("mail_body"), rs.getString("mail_status"), rs.getLong("created_by"),
                rs.getString("created_at"), rs.getString("completed_at")), task.id()).get(0);
    }
    ...
    public List<MailLogDetail> logs(long manager, long taskId) {
        ...
        return jdbc.query("""
                ...
                """, (rs, row) -> new MailLogDetail(
                rs.getLong("id"), rs.getLong("mail_task_id"), rs.getString("recipient_name"),
                rs.getString("recipient_email"), rs.getString("mail_status"), rs.getInt("retry_count"),
                rs.getString("error_message"), rs.getString("provider_message_id"), rs.getString("sent_at"),
                rs.getString("created_at"), rs.getString("updated_at")), taskId);
    }

    private MailTask task(long manager, long id) {
        MailTask task = jdbc.query("""
                SELECT id,report_id,subject,mail_body,mail_status,created_by
                  FROM mail_task WHERE id=?
                """, (rs, row) -> new MailTask(
                rs.getLong("id"), rs.getLong("report_id"), rs.getString("subject"),
                rs.getString("mail_body"), rs.getString("mail_status"), rs.getLong("created_by")), id)
                .stream().findFirst().orElseThrow(() -> new DepartmentBusinessException(404, "邮件任务不存在"));
        if (task.createdBy() != manager) throw new DepartmentBusinessException(403, "无权操作其他用户创建的邮件任务");
        return task;
    }
    ...
    private List<MailRecipientLog> recipientLogs(long taskId) {
        return jdbc.query("""
                SELECT id,mail_task_id,recipient_name,recipient_email,mail_status
                  FROM mail_log WHERE mail_task_id=? ORDER BY id
                """, (rs, row) -> new MailRecipientLog(
                rs.getLong("id"), rs.getLong("mail_task_id"), rs.getString("recipient_name"),
                rs.getString("recipient_email"), rs.getString("mail_status")), taskId);
    }
    ...
    private MailRecipientLog recipientLog(long id) {
        return jdbc.query("""
                ...
                """, (rs, row) -> new MailRecipientLog(
                rs.getLong("id"), rs.getLong("mail_task_id"), rs.getString("recipient_name"),
                rs.getString("recipient_email"), rs.getString("mail_status")), id)
                .stream().findFirst().orElseThrow(() -> new DepartmentBusinessException(404, "邮件日志不存在"));
    }
    ...
    public record CreateRequest(long reportId, String subject, String mailBody, List<Recipient> recipients) {}
    ...
    public record SendResult(long mailTaskId, String status, int totalCount, int successCount, int failedCount,
                             List<AttachmentInfo> attachments, List<MailLogDetail> logs) {}
    public record MailTaskDetail(long id, long reportId, String subject, String mailBody, String status,
                                 long createdBy, String createdAt, String completedAt) {}
    public record MailLogDetail(long id, long mailTaskId, String recipientName, String recipientEmail,
                                String status, int retryCount, String errorMessage, String providerMessageId,
                                String sentAt, String createdAt, String updatedAt) {}
    private record MailTask(long id, long reportId, String subject, String body, String status, long createdBy) {}
