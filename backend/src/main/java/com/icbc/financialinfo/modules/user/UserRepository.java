...
public class UserRepository {
    ...
    private static final RowMapper<UserAccount> USER_MAPPER = (rs, rowNum) -> new UserAccount(
            ...
            rs.getLong("role_id"), rs.getInt("status"), rs.getString("created_at"),
    ...
    public Optional<UserAccount> findByUsername(String username) {
        return jdbcTemplate.query(SELECT_COLUMNS + " WHERE u.username = ?", USER_MAPPER, username)
                .stream().findFirst();
    }

    public Optional<UserAccount> findById(long id) {
        return jdbcTemplate.query(SELECT_COLUMNS + " WHERE u.id = ?", USER_MAPPER, id)
                .stream().findFirst();
    }

    public List<UserAccount> findPage(int pageNum, int pageSize, String username, Long roleId, Integer status) {
        QueryParts query = filters(username, roleId, status);
        query.parameters().add(pageSize);
        query.parameters().add((pageNum - 1) * pageSize);
        return jdbcTemplate.query(SELECT_COLUMNS + query.whereClause() + " ORDER BY u.id LIMIT ? OFFSET ?",
                USER_MAPPER, query.parameters().toArray());
    }

    public long count(String username, Long roleId, Integer status) {
        QueryParts query = filters(username, roleId, status);
        Long count = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM sys_user u" + query.whereClause(), Long.class,
    ...
    public boolean usernameExists(String username) {
        Integer count = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM sys_user WHERE LOWER(username) = LOWER(?)", Integer.class, username);
    ...
    public boolean update(long id, long roleId, int status) {
        return jdbcTemplate.update("UPDATE sys_user SET role_id = ?, status = ? WHERE id = ?",
                roleId, status, id) > 0;
    }
    ...
    public record UserAccount(Long id, String username, String passwordHash, Long roleId,
                              Integer status, String createdAt, String roleCode, String roleName) {}
