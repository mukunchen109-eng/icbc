...
public final class ReportTextFormatter {
  ...
  private static boolean appendStructuredBlocks(JsonNode node, List<Block> blocks) {
    ...
    hasStructuredSection |= appendSectionFromArray(
  ...
  private static boolean appendSectionFromArray(
    JsonNode node,
    String sectionTitle,
    List<Block> blocks,
  ...
  private static List<Block> compressBlankBlocks(List<Block> blocks) {
    ...
    for (Block block : blocks) {
      if (
        ...
        !result.isEmpty() &&
        result.get(result.size() - 1).type() == BlockType.BLANK
      ) {
        continue;
      }
      result.add(block);
