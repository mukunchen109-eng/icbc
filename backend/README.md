...
邮件附带最终报告的 PDF 和 Word 文件。报告、原始资讯 CSV、审核日志、邮件日志及归档 ZIP 默认保存在 `backend/data/archives`，可用 `ARCHIVE_ROOT` 修改目录。可用 `REPORT_FONT_PATH` 指定支持中文的正文字体，并用 `REPORT_TITLE_FONT_PATH` 指定标题字体；Windows 默认分别使用仿宋和黑体。
